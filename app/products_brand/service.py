...
class ProductBrandService:
    ...
    # CREATE
    # ----------------------
    def create_product_category(self, item_data: ProductBrandCreate, session: SessionDep):
    ...
    # UPDATE
    # ----------------------
    def update_product_category(self, item_id: int, item_data: ProductBrandUpdate, session: SessionDep):
        product_db = session.get(ProductBrand, item_id)
        if not product_db:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=self.no_task
